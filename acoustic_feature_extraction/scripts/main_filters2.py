...
'''
This is the equalisation node & sound filtering node
...
class FilterNode:
    ...
    def _audio_cb(self, msg):	
        ...
        nft = self._sample_rate                #########?????
                

                    # convert the audio to numpy array
        audio_data = msg.data
        audio_data_numpy = np.frombuffer(audio_data, dtype=np.int16)		
        origFFT = fft(audio_data_numpy)		         #turn this into amplitude against freq graph
                
        #amplitude of each frequency range
        nobVal = [0,0,0,0,0,0,0,0]                   #array of random amplitudes
                
        Gain=0; #gain
                
        #normalizing the nobs from range of 0 to 10 to range of 0 to 1 by dividing each nob value by 10.
        nobVal[:] = [i/10 for i in nobVal]
                
        #frequency array
        freq = [32,64,128,256,512,1000,2000,3000]		#array of random frequencies to simulate the freq that are gg to be targeted at filtering stage
                
        #initializing an empty array
        bin = [0] * 8
                
        for i in range (0,8):
            bin[i]= int((freq[i]*nft)/self._sample_rate)        ##########what is this for? upscaling

        #sdownscaling the frequencies
        #freq[:] = [i*self._sample_rate/nft for i in (0,nft-1)]			#not needed actually in our case
                
                
        #defining the end of the spectrum since it will be required due to symmetry.
        #end = int(origFFT[-1])
                
        #equalising according to the different freq range that bin extends to     --> just bringing down all the freq
        for i in range (0,7):

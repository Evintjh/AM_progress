...
PidObject::PidObject() : error_(3, 0), filtered_error_(3, 0)  
{
  ...
  // defaults
  node_priv.param<double>("Kp", Kp_, 1.0);                                                              //(para name from launch file, name used/called in this file, defualt value)
  ...
  node_priv.param<std::string>("topic_from_controller", topic_from_controller_, "control_effort");              //published
  node_priv.param<std::string>("topic_from_plant", topic_from_plant_, "/control/power");                        //published now
...
void PidObject::doCalcs()
{
  ...
  if (new_state_or_setpt_)
  {
    ...
    error_.at(0) = setpoint_ - rms_energy;  // Current error goes to slot 0
    ...
    { 
      ...
      while (error_.at(0) < -((rms_energy)*10**5) / 2.0)     
      {
        error_.at(0) += ((rms_energy)*10**5);                                  //means plant state > setpoint                                         //error varies with AFE::rms_energy now
      ...
      while (error_.at(0) > ((rms_energy)*10**5) / 2.0)                            //rms_energy value is too small, about 10**-5
      {
        error_.at(0) -= ((rms_energy)*10**5);
